...
if ! grep -q "${aws_instance.prj-vm[0].public_ip}" ansible/inventory/hosts 2>/dev/null; then
  if ! grep -q "^\[bitbucket\]" ansible/inventory/hosts 2>/dev/null; then
    echo "[bitbucket]" >> ansible/inventory/hosts
  fi
...
resource "null_resource" "run_bitbucket_setup_playbook" {
  depends_on = [
  ...
  triggers = {
    ...
    roles_hash    = filemd5("${path.root}/ansible/roles/bitbucket/tasks/main.yml")
  }
